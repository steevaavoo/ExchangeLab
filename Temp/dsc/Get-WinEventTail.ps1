...
Function Get-WinEventTail {
    ...
    Begin {
        Write-Verbose -Message "Tailing Win Events for [$ComputerName]"
        if ($ShowExisting -gt 0) {
            $events = Get-WinEvent -LogName $LogName -MaxEvents $ShowExisting -ComputerName $ComputerName -Credential $Credential -ErrorAction 'SilentlyContinue'
            $events | Sort-Object -Property 'RecordId'
            $firstRecordId = if (-not $null -eq $events) {$events[0].RecordId}
        }
        else {
